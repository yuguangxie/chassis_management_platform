    ...
    <section class="top-row">
      <article class="panel directory-panel">
        ...
        <div class="directory-path">
          ...
          <button v-if="auth.isAdmin" type="button" :disabled="reportsStore.offline" @click="runAction('change-directory', '更改目录')">更改目录</button>
        </div>
    ...
    <section class="bottom-row">
      ...
      <article class="panel operations-panel">
        ...
        <div class="operation-grid">
          <button v-for="item in visibleOperationButtons" :key="item.action" :class="{ danger: item.action === 'delete' }" type="button" :disabled="reportsStore.offline || !selected.report_id" @click="handleOperation(item.action, item.label)">
...
const operationButtons: Array<{ label: string; action: OperationAction; icon: Component; role: Role }> = [
  { label: '导出Word', action: 'export-word', icon: FileText, role: 'operator' }, { label: '导出PDF', action: 'export-pdf', icon: FileOutput, role: 'operator' },
  { label: '打印', action: 'print', icon: Printer, role: 'operator' }, { label: '删除', action: 'delete', icon: Trash2, role: 'admin' },
  { label: '重新生成', action: 'regenerate', icon: RotateCcw, role: 'operator' }, { label: '查看关联日志', action: 'related-logs', icon: ScrollText, role: 'viewer' },
]
...
.chart-panel { display:grid;grid-template-rows:32px minmax(0,1fr);padding:0 8px 8px; }.chart-panel header { display:flex;align-items:center;justify-content:space-between; }.chart-panel select { height:23px;border:1px solid #315A83;border-radius:4px;color:#AFC2DA;background:#081A2F;font-size:9px;padding:0 5px; }.operations-panel { display:grid;grid-template-rows:28px minmax(0,1fr) auto;padding:0 10px 8px; }.operations-panel h2 { display:flex;align-items:center; }.operation-grid { display:grid;grid-template-columns:1fr 1fr;grid-auto-rows:minmax(28px,1fr);gap:6px; }.operation-grid button { display:flex;align-items:center;justify-content:center;gap:8px;border:1px solid #2F80FF;border-radius:6px;color:#EAF2FF;background:linear-gradient(135deg,#176CD9,#0E4A98);font-size:11px;font-weight:700; }.operation-grid button.danger { border-color:#EF4444;background:linear-gradient(135deg,#B42534,#721B29); }.print-job{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:4px 6px;margin-top:5px;padding-top:5px;border-top:1px solid #1E3A5F;font-size:8px}.print-job b,.print-job small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.print-job small{grid-column:1/4;color:#8098B7}.print-job em{grid-column:1/4;color:#EF4444;font-style:normal}.print-job button{height:20px;border:1px solid #315A83;border-radius:3px;color:#DDEBFF;background:#102A4A;font-size:8px}.print-state{padding:2px 5px;border:1px solid #315A83;border-radius:3px;color:#8FC2FF}.print-state.completed{color:#21C55D;border-color:#21C55D}.print-state.failed{color:#EF4444;border-color:#EF4444}.print-state.cancelled{color:#F6C343;border-color:#F6C343}
