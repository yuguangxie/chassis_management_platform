<template>
  <section :class="['report-page', previewFullscreen ? 'preview-fullscreen' : '']">
    <header class="page-title-row">
      ...
      <span v-if="reportsStore.offline || dashboard.mock || dashboard.quality !== 'good'" class="mock-badge">{{ reportsStore.offline ? '后端离线，当前使用 Mock 报告数据' : dashboard.mock ? '显式 Mock 报告数据' : `数据质量：${dashboard.quality || 'unavailable'}` }}</span>
    </header>
    ...
    <section class="top-row">
      <article class="panel directory-panel">
        ...
        <div class="directory-path">
          ...
          <button type="button" @click="runAction('change-directory', '更改目录')">更改目录</button>
        </div>
        ...
        <footer><span>最近扫描时间　{{ dashboard.directory.last_scan_time }}</span><b>{{ dashboard.directory.scan_status }}</b></footer>
      </article>
      ...
      <article class="panel filter-panel">
        ...
        <div class="filter-content">
          <div class="filters-grid">
            ...
            <label><span>结果</span><select v-model="filters.result"><option>全部</option><option>PASS</option><option>FAIL</option></select></label>
            <label><span>操作员</span><select v-model="filters.operator"><option>全部</option><option>op01</option><option>op02</option></select></label>
            <label><span>文件类型</span><select v-model="filters.fileType"><option>全部</option><option>.docx</option><option>.pdf</option><option>.json</option><option>.csv</option><option>raw.log</option></select></label>
          </div>
          <div class="scan-actions">
            <button type="button" @click="runAction('scan', '扫描目录')"><RefreshCw :size="19" /><span><strong>扫描目录</strong><small>刷新目录报告索引</small></span></button>
            <button type="button" @click="runAction('open-directory', '打开报告目录')"><FolderOpen :size="19" /><span><strong>打开报告目录</strong><small>定位当前报告目录</small></span></button>
          </div>
        </div>
      </article>
    </section>
    ...
    <section class="middle-row">
      <article class="panel report-list-panel">
        <header class="panel-header"><h2>报告列表</h2><div class="table-tools"><button aria-label="刷新报告列表" @click="reportsStore.loadDashboard()"><RefreshCw :size="14" /></button><button aria-label="列设置"><Columns3 :size="14" /></button><button aria-label="列表密度"><List :size="14" /></button></div></header>
          ...
          <table class="dense-table report-table">
            ...
            <tbody>
              <tr v-for="item in filteredReports" :key="item.report_id" :class="{ selected: item.report_id === selected.report_id }" @click="selectReport(item)">
                ...
                <td><span class="complete-pill">{{ item.generation_status }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <article class="panel preview-panel">
        ...
        <div v-if="activePreviewTab === '预览'" class="document-viewer">
          <div class="viewer-toolbar">
            <button aria-label="预览菜单"><Menu :size="15" /></button><strong>{{ selected.filename }}</strong>
            ...
            <button aria-label="打印报告" @click="runAction('print', '打印')"><Printer :size="14" /></button>
          </div>
          <div class="paper-viewport">
            ...
            <div v-else class="report-paper" :style="{ transform: `scale(${zoom / 100})` }">
              ...
              <dl><div><dt>底盘编号：</dt><dd>{{ selected.preview.chassis_no }}</dd></div><div><dt>VIN：</dt><dd>{{ selected.preview.vin }}</dd></div><div><dt>检测时间：</dt><dd>{{ selected.preview.test_time }}</dd></div><div><dt>结果：</dt><dd :class="selected.preview.result.toLowerCase()">{{ selected.preview.result }}</dd></div><div><dt>操作员：</dt><dd>{{ selected.preview.operator }}</dd></div></dl>
              <div :class="['pass-stamp', selected.preview.result.toLowerCase()]">{{ selected.preview.result }}</div>
            </div>
          </div>
        </div>
        <pre v-else-if="activePreviewTab === 'JSON摘要'" class="code-preview">{{ JSON.stringify(selected.json_summary || {}, null, 2) }}</pre>
        <div v-else class="csv-preview"><table class="dense-table"><thead><tr><th>信号</th><th>值</th><th>单位</th><th>结果</th></tr></thead><tbody><tr v-for="row in selected.csv_rows || []" :key="String(row.signal)"><td>{{ row.signal }}</td><td>{{ row.value }}</td><td>{{ row.unit }}</td><td>{{ row.result }}</td></tr></tbody></table></div>
      </article>
    </section>
    ...
    <section class="bottom-row">
      <article class="panel related-panel">
        <h2>关联数据</h2>
        <div class="data-tabs"><button v-for="(tabName, index) in relatedTabs" :key="tabName" :class="{ active: index === 0 }">{{ tabName }}</button></div>
        <div class="table-scroll"><table class="dense-table related-table"><thead><tr><th>Session ID</th><th>开始时间</th><th>结束时间</th><th>结果</th><th>报告数</th><th>操作</th></tr></thead><tbody><tr v-for="row in dashboard.related_data.sessions" :key="row.session_id"><td>{{ row.session_id }}</td><td>{{ row.started_at }}</td><td>{{ row.ended_at }}</td><td><StatusPill :value="row.result" /></td><td>{{ row.report_count }}</td><td><button class="eye-btn" aria-label="查看会话"><Eye :size="14" /></button></td></tr></tbody></table></div>
      </article>

      <article class="panel chart-panel"><header><h2>报告存储使用趋势</h2><select><option>近7天</option></select></header><RealtimeLineChart :option="storageTrendOption" height="100%" /></article>
      <article class="panel chart-panel"><header><h2>结果分布统计</h2><select><option>全部时间</option></select></header><RealtimeLineChart :option="resultDistributionOption" height="100%" /></article>

      <article class="panel operations-panel">
        <h2>操作</h2>
        <div class="operation-grid">
          <button v-for="item in operationButtons" :key="item.action" :class="{ danger: item.action === 'delete' }" type="button" @click="handleOperation(item.action, item.label)">
...
const operationButtons: Array<{ label: string; action: OperationAction; icon: Component }> = [
  { label: '导出Word', action: 'export-word', icon: FileText }, { label: '导出PDF', action: 'export-pdf', icon: FileOutput },
  { label: '打印', action: 'print', icon: Printer }, { label: '删除', action: 'delete', icon: Trash2 },
  { label: '重新生成', action: 'regenerate', icon: RotateCcw }, { label: '查看关联日志', action: 'related-logs', icon: ScrollText },
]
...
const storageTrendOption = computed(() => {
  ...
  return { backgroundColor: 'transparent', color: ['#2F80FF', '#21C55D'], tooltip: { trigger: 'axis' }, legend: { bottom: 0, textStyle: { color: '#AFC2DA', fontSize: 10 }, itemWidth: 16 }, grid: { left: 32, right: 12, top: 18, bottom: 36, containLabel: true }, xAxis: { type: 'category', data: rows.map((row) => row.date), boundaryGap: false, axisLabel: { color: '#8CA6C5', fontSize: 10 }, axisLine: { lineStyle: { color: '#315A83' } } }, yAxis: { type: 'value', name: 'GB', nameTextStyle: { color: '#8CA6C5' }, axisLabel: { color: '#8CA6C5', fontSize: 10 }, splitLine: { lineStyle: { color: '#1A385C', type: 'dashed' } } }, series: [
...
const resultDistributionOption = computed(() => {
  ...
  return { backgroundColor: 'transparent', tooltip: { trigger: 'item' }, legend: { orient: 'vertical', right: 4, top: 'center', textStyle: { color: '#CFE2FF', fontSize: 11 }, itemWidth: 9, itemHeight: 9, formatter: (name: string) => { const row = rows.find((item) => item.name === name); return `${name}  ${(row?.value || 0).toLocaleString()} (${(row?.percent || 0).toFixed(1)}%)` } }, graphic: [{ type: 'text', left: '26%', top: '43%', style: { text: `总计\n${total.toLocaleString()}`, fill: '#EAF2FF', fontSize: 16, fontWeight: 700, textAlign: 'center', lineHeight: 21 } }], series: [{ type: 'pie', radius: ['48%', '70%'], center: ['28%', '54%'], label: { show: false }, data: rows.map((row) => ({ name: row.name, value: row.value, itemStyle: { color: row.name === 'PASS' ? '#21C55D' : '#EF4444' } })) }] }
})
...
onMounted(async () => {
  ...
  pollTimer = window.setInterval(() => void reportsStore.loadDashboard(), 5000)
})
...
<style scoped>
.report-page { position:relative; height:100%; min-height:0; overflow:hidden; display:grid; grid-template-rows:52px 200px 340px minmax(0,1fr); gap:10px; color:#EAF2FF; }
...
.chart-panel { display:grid;grid-template-rows:32px minmax(0,1fr);padding:0 8px 8px; }.chart-panel header { display:flex;align-items:center;justify-content:space-between; }.chart-panel select { height:23px;border:1px solid #315A83;border-radius:4px;color:#AFC2DA;background:#081A2F;font-size:9px;padding:0 5px; }.operations-panel { display:grid;grid-template-rows:32px minmax(0,1fr);padding:0 10px 10px; }.operations-panel h2 { display:flex;align-items:center; }.operation-grid { display:grid;grid-template-columns:1fr 1fr;grid-template-rows:repeat(3,1fr);gap:10px; }.operation-grid button { display:flex;align-items:center;justify-content:center;gap:10px;border:1px solid #2F80FF;border-radius:6px;color:#EAF2FF;background:linear-gradient(135deg,#176CD9,#0E4A98);font-size:13px;font-weight:700; }.operation-grid button.danger { border-color:#EF4444;background:linear-gradient(135deg,#B42534,#721B29); }
...
@media(max-height:800px){.report-page{grid-template-rows:52px 190px 320px 270px;overflow:auto;scrollbar-color:#2B5C90 #07172A}.directory-panel{grid-template-rows:24px 42px 104px 20px}}
</style>
