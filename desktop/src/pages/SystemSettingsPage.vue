  ...
  <section class="system-settings-page">
    <header class="page-title-row">
      <div class="title-left">
        ...
        <div><h1>系统设置</h1><p>阈值、存储、DBC、权限、模拟网关与维护模式</p></div>
      </div>
      <div class="title-actions">
        ...
        <span v-if="auth.principal" class="status-pill saved"><span></span>{{ auth.principal.username }} / {{ localizeStatus(auth.principal.role) }}</span>
        ...
        <IndustrialButton v-if="auth.can('engineer')" compact variant="primary" :disabled="hasInvalidThreshold || directConfigDisabled" :title="signedAuthority ? '生产配置必须通过签名包导入和应用' : ''" @click="saveSettings"><template #icon><Save :size="14" /></template>保存设置</IndustrialButton>
      </div>
    </header>

    <section class="settings-row-one" :inert="signedAuthority" :aria-disabled="signedAuthority ? 'true' : 'false'">
      <article class="settings-card basic-card">
        ...
        <div class="field-grid card-scroll">
          ...
          <label><span>默认控制通道</span><select v-model="settings.basic.control_channel" title="安全策略锁定：CAN1 禁止作为控制通道" @change="markDirty"><option value="CAN1" disabled>CAN1（安全锁定）</option><option value="CAN2">CAN2（批准）</option></select></label>
          ...
          <div class="field-toggle"><span>自动保存配置</span><ToggleSwitch v-model="settings.basic.auto_save" label="自动保存配置" @update:model-value="markDirty" /></div>
        </div>
      </article>
      ...
      <article class="settings-card dbc-card">
        ...
        <div class="dbc-content">
          <div class="dbc-meta">
            <div><span>当前文件</span><strong :title="settings.dbc.filename || '未知'">{{ settings.dbc.filename || '未知' }}</strong></div>
            <div><span>版本 / Hash</span><strong>{{ settings.dbc.version || '未知' }} · {{ settings.dbc.hash || '未知' }}</strong></div>
            ...
            <div class="full"><span>最近加载</span><strong>{{ settings.dbc.loaded_at || '未知' }}</strong></div>
          </div>
      ...
      <article class="settings-card storage-card">
        ...
        <div class="storage-content card-scroll">
          ...
          <div class="format-toggles"><span title="清理必须由管理员预检并确认">自动清理：已禁用</span><ToggleSwitch v-model="settings.storage.word_enabled" label="Word" @update:model-value="markDirty" /><ToggleSwitch v-model="settings.storage.pdf_enabled" label="PDF" @update:model-value="markDirty" /><ToggleSwitch v-model="settings.storage.csv_enabled" label="CSV" @update:model-value="markDirty" /><ToggleSwitch v-model="settings.storage.parquet_enabled" label="Parquet" @update:model-value="markDirty" /></div>
    ...
    <section class="settings-row-two">
      ...
      <article class="settings-card audit-card">
        <CardTitle number="5" title="权限与审计"><ShieldCheck :size="16" /><template #meta><span class="admin-badge">管理员审批</span></template></CardTitle>
        <div class="audit-content">
          <div class="table-scroll role-scroll"><table class="dense-table role-table"><thead><tr><th>角色</th><th>查看</th><th>检测</th><th>手动控制</th><th>配置</th><th>维护</th><th>删除报告</th></tr></thead><tbody><tr v-for="role in settings.roles" :key="role.role" :class="{ admin:role.role==='admin' }"><td><span :class="['role-pill',role.role]">{{ localizeStatus(role.role) }}</span></td><td>{{ role.view }}</td><td>{{ role.test }}</td><td>{{ role.manual_control }}</td><td>{{ role.config }}</td><td>{{ role.maintenance }}</td><td><span :class="['yes-no',role.delete_report?'yes':'no']">{{ role.delete_report?'是':'否' }}</span></td></tr></tbody></table></div>
          <div class="subhead"><History :size="13" />最近配置历史 <span>修改权限配置必须经过后端审批</span></div>
          <div class="table-scroll history-scroll"><table class="dense-table config-history-table"><thead><tr><th>时间</th><th>用户</th><th>配置项</th><th>旧值</th><th>新值</th><th>原因</th><th>结果</th></tr></thead><tbody><tr v-for="row in settings.config_history" :key="`${row.time}-${row.key}`"><td>{{ row.time }}</td><td><span :class="['history-user',row.user.includes('admin')&&'admin']">{{ row.user }}</span></td><td>{{ row.key }}</td><td>{{ row.old_value }}</td><td>{{ row.new_value }}</td><td>{{ row.reason }}</td><td><span class="success-pill">{{ localizeStatus(row.result) }}</span></td></tr></tbody></table></div>
        </div>
      </article>
    </section>
    ...
    <section class="settings-row-three">
      <article class="settings-card maintenance-card">
        ...
        <div class="maintenance-content">
          ...
          <div v-if="auth.isAdmin" class="maintenance-actions"><button class="mock" :disabled="writeDisabled" @click="openDialog('mock')"><Bot :size="14" />{{ settings.maintenance.mock_can_gateway?'关闭模拟网关（Mock）':'启用模拟网关（Mock）' }}</button><button :class="settings.maintenance.maintenance_mode?'exit':'enter'" :disabled="writeDisabled" @click="toggleMaintenance"><ShieldAlert :size="14" />{{ settings.maintenance.maintenance_mode?'退出维护模式':'进入维护模式' }}</button></div>
        </div>
      </article>
      ...
      <article class="settings-card trend-card">
        ...
        <div class="trend-content"><RealtimeLineChart :option="storageTrendOption" height="132px" /><div class="storage-kpis"><div><span>当前日志</span><strong>{{ settings.storage_summary.current_log_gb }} GB</strong></div><div><span>数据库</span><strong>{{ settings.storage_summary.database_gb }} GB</strong></div><div><span>报告</span><strong>{{ settings.storage_summary.reports_gb }} GB</strong></div><div><span>原始 CAN</span><strong>{{ settings.storage_summary.raw_can_gb }} GB</strong></div></div><div class="cleanup-line"><CheckCircle2 :size="13" /><span>最近清理 {{ settings.storage_summary.last_cleanup }}</span><span>下次 {{ settings.storage_summary.next_cleanup }}</span><b>{{ localizeStatus(settings.storage_summary.cleanup_status) }} / 磁盘{{ localizeStatus(settings.storage_summary.disk_alarm) }}</b></div></div>
      </article>

      <article class="settings-card safety-card">
        <CardTitle number="8" title="安全默认值与系统版本"><LockKeyhole :size="16" /><template #meta><span :class="['acceptance-pill', settings.hardware_acceptance.allowed ? 'approved' : 'blocked']" :title="hardwareAcceptanceTitle">硬件验收：{{ settings.hardware_acceptance.allowed ? '已批准' : '阻断' }}</span></template></CardTitle>
        <div class="safety-version-grid">
          ...
          <div class="version-list"><div><span>运行配置</span><b>{{ settings.runtime_profile }} / {{ settings.configuration_authority }}</b></div><div><span>软件版本</span><b>{{ settings.version.software || '未知' }}</b></div><div><span>配置版本</span><b>{{ settings.version.config || '未知' }}</b></div><div><span>检测方案</span><b>{{ settings.version.test_plan || '未知' }}</b></div><div><span>发布 Hash</span><b>{{ settings.version.release_hash || '未知' }}</b></div><div><span>提交</span><b>{{ settings.version.commit || '未知' }}</b></div><div><span>Python / Node</span><b>{{ settings.version.python || '未知' }} / {{ settings.version.node || '不适用' }}</b></div><div><span>Electron</span><b>{{ settings.version.electron || '未知' }}</b></div><div><span>构建时间</span><b>{{ settings.version.build_time || '未知' }}</b></div></div>
        </div>
    ...
    <footer class="settings-action-bar">
      <IndustrialActionButton v-if="auth.can('engineer')" variant="primary" title="保存设置" :subtitle="signedAuthority ? '生产配置仅允许签名包应用' : '保存所有配置变更'" :disabled="directConfigDisabled" @click="saveSettings"><template #icon><Save :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton v-if="auth.isAdmin" title="导入配置" subtitle="校验、预览后应用" :disabled="writeDisabled" @click="chooseConfigFile"><template #icon><Upload :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton v-if="auth.isAdmin" title="导出配置" subtitle="导出签名配置包" :disabled="writeDisabled" @click="exportConfig"><template #icon><Download :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton v-if="auth.isAdmin" title="一致性备份" subtitle="在线备份并校验哈希" :disabled="writeDisabled" @click="createBackup"><template #icon><Database :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton v-if="auth.isAdmin" variant="warning" title="恢复数据库备份" subtitle="校验后保留回滚副本" :disabled="writeDisabled" @click="restoreLatestBackup"><template #icon><RotateCcw :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton v-if="auth.isAdmin" variant="warning" title="清理预检" subtitle="先 dry-run 再确认" :disabled="writeDisabled" @click="previewCleanup"><template #icon><HardDrive :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton v-if="auth.isAdmin" title="存储复检" subtitle="检查路径、空间与数据库" :disabled="writeDisabled" @click="recheckStorage"><template #icon><Activity :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton title="重载 DBC" :subtitle="signedAuthority ? '生产 DBC 由签名配置包固定' : '重新扫描并加载资源'" :disabled="directConfigDisabled" @click="reloadDbc"><template #icon><RefreshCw :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton v-if="auth.isAdmin" variant="warning" :title="settings.maintenance.mock_can_gateway?'关闭模拟网关':'启用模拟网关'" subtitle="切换 MockCanGateway" :disabled="writeDisabled" @click="openDialog('mock')"><template #icon><Bot :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton v-if="auth.isAdmin" variant="warning" :title="settings.maintenance.maintenance_mode?'退出维护模式':'进入维护模式'" subtitle="需管理员二次确认" :disabled="writeDisabled" @click="toggleMaintenance"><template #icon><ShieldAlert :size="23" /></template></IndustrialActionButton>
      <IndustrialActionButton v-if="auth.isAdmin" variant="danger" title="恢复安全默认" :subtitle="signedAuthority ? '生产配置仅允许签名包应用' : '仅恢复安全相关参数'" :disabled="directConfigDisabled" @click="openDialog('restore')"><template #icon><RotateCcw :size="23" /></template></IndustrialActionButton>
    </footer>
...
const signedAuthority = computed(() => settings.value.configuration_authority === 'signed-package')
const directConfigDisabled = computed(() => writeDisabled.value || settings.value.configuration_authority === 'signed-package')
const toast = ref('')
...
const featureRows: FeatureRow[] = [
  {key:'mock_can_gateway',label:'模拟 CAN 网关（Mock）',note:'必须明确显示模拟模式',risk:true,icon:Bot},
...
const dbcStatusLabel = computed(() => settings.value.dbc.status==='loaded'?'已加载':settings.value.dbc.status==='raw-only'?'仅原始帧模式':'加载失败')
const hardwareAcceptanceTitle = computed(() => {
  const artifact = settings.value.hardware_acceptance.artifact
  const scope = artifact ? `工位 ${artifact.station_id}；车型 ${artifact.vehicle_series}；有效期至 ${artifact.valid_to}` : '未加载验收 artifact'
  const reasons = (settings.value.hardware_acceptance?.reasons || []).map(item => `${item.label}（${item.rule}）`).join('；')
  return `${scope}${reasons ? `；阻断：${reasons}` : ''}`
})
...
const dialogTitle = computed(() => ({save:'保存危险参数',maintenance:'进入维护模式',mock:settings.value.maintenance.mock_can_gateway?'关闭模拟 CAN 网关（Mock）':'启用模拟 CAN 网关（Mock）',restore:'恢复安全默认值',feature:`变更 ${pendingFeature.value?.label||'危险功能'}`}[dialogMode.value||'save']))
const dialogImpact = computed(() => ({save:'危险阈值会影响车辆控制与检测判定，后端将再次校验范围和权限。',maintenance:'维护模式可能导致车辆异常运动，仅允许管理员在封闭台架中使用。',mock:'模拟模式不得伪装为真实硬件，切换将同步顶部状态并写入审计。',restore:'仅恢复速度、转角、告警、扩展报文和控制通道等安全参数，保留工位号与路径。',feature:`${pendingFeature.value?.note||''}，启用前必须确认维护模式与现场安全条件。`}[dialogMode.value||'save']))
const storageTrendOption = computed(() => {
  const trend = settings.value.storage_trend || []
  return {backgroundColor:'transparent',color:['#2F80FF'],tooltip:{trigger:'axis'},grid:{left:35,right:12,top:14,bottom:24,containLabel:true},xAxis:{type:'category',data:trend.map(item=>item.date),axisLabel:{color:'#8CA6C5',fontSize:9,hideOverlap:true},axisLine:{lineStyle:{color:'#315A83'}}},yAxis:{type:'value',name:'GB',nameTextStyle:{color:'#8CA6C5',fontSize:9},axisLabel:{color:'#8CA6C5',fontSize:9,hideOverlap:true},splitLine:{lineStyle:{color:'#1B3A5E',type:'dashed'}}},series:[{name:'日志空间',type:'bar',barWidth:'44%',data:trend.map(item=>item.used_gb),itemStyle:{color:'#2F80FF',borderRadius:[3,3,0,0]},label:{show:true,position:'top',color:'#CFE2FF',fontSize:9}}]}
})
...
.safety-card{position:relative}.acceptance-pill{max-width:170px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding:2px 6px;border:1px solid;border-radius:4px;font-size:8px}.acceptance-pill.approved{color:#21C55D;border-color:rgba(33,197,93,.45);background:rgba(33,197,93,.08)}.acceptance-pill.blocked{color:#EF4444;border-color:rgba(239,68,68,.45);background:rgba(239,68,68,.08)}.safety-version-grid{height:calc(100% - 67px);display:grid;grid-template-columns:1.05fr .95fr;gap:9px;padding-top:5px}.safe-rules,.version-list{min-height:0;display:grid}.safe-rules{grid-template-rows:repeat(9,1fr);border-right:1px solid #1E3A5F;padding-right:8px}.safe-rules>div{display:grid;grid-template-columns:16px 1fr 28px;align-items:center;border-bottom:1px solid #153153;color:#21C55D;font-size:8px}.safe-rules span{color:#B8C9DC}.safe-rules b{font-size:8px}.version-list{grid-template-rows:repeat(9,1fr)}.version-list>div{display:grid;grid-template-columns:65px 1fr;align-items:center;border-bottom:1px solid #153153;font-size:8px}.version-list span{color:#7F97B6}.version-list b{overflow:hidden;color:#DDEBFF;text-overflow:ellipsis;white-space:nowrap}.safety-actions{position:absolute;left:10px;right:10px;bottom:7px;display:grid;grid-template-columns:1fr 1fr;gap:6px}.safety-actions .danger-outline{color:#F6C343;border-color:#B17D1D;background:#30250D}
.settings-action-bar{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));grid-auto-rows:70px;align-content:start;gap:6px 9px;min-height:0}.settings-action-bar :deep(.industrial-action){width:100%;padding-inline:8px;gap:7px}.settings-action-bar :deep(.icon-block){width:36px}.settings-action-bar :deep(.action-copy strong){font-size:11px}.settings-action-bar :deep(.action-copy small){font-size:8px}
...
@media(max-height:800px){.system-settings-page{grid-template-rows:42px minmax(125px,1fr) minmax(125px,1fr) minmax(115px,1fr) 146px;gap:7px;overflow:hidden}.settings-action-bar{gap:6px}.settings-action-bar :deep(.action-copy small){display:none}.card-title{height:27px}.field-grid{height:calc(100% - 27px);grid-auto-rows:26px}.dbc-content,.storage-content,.audit-content{height:calc(100% - 27px)}}
</style>
