  ...
  <section class="system-settings-page">
    <header class="page-title-row">
      ...
      <div class="title-actions">
        <span v-if="settingsStore.offline" class="offline-pill">后端离线，当前使用 Mock 系统配置</span>
        ...
        <button @click="importConfig"><Upload :size="14" />导入配置</button>
        <button @click="exportConfig"><Download :size="14" />导出配置</button>
        <button class="primary" :disabled="hasInvalidThreshold" @click="saveSettings"><Save :size="14" />保存设置</button>
      </div>
    </header>
    ...
    <section class="settings-row-one">
      <article class="settings-card basic-card">
        ...
        <div class="field-grid card-scroll">
          ...
          <label class="wide"><span>默认报告目录</span><div class="path-input"><input v-model="settings.basic.report_directory" @input="syncReportPath" /><button aria-label="选择默认报告目录" @click="pathStub('默认报告目录')"><FolderOpen :size="14" /></button></div></label>
          <label class="wide"><span>SQLite 数据库路径</span><div class="path-input"><input v-model="settings.basic.database_path" @input="syncDatabasePath" /><button aria-label="选择数据库文件" @click="pathStub('数据库文件')"><Database :size="14" /></button></div></label>
          <label class="wide"><span>日志根目录</span><div class="path-input"><input v-model="settings.basic.log_directory" @input="markDirty" /><button aria-label="选择日志目录" @click="pathStub('日志根目录')"><FolderOpen :size="14" /></button></div></label>
      ...
      <article class="settings-card dbc-card">
        ...
        <div class="mini-actions"><button @click="pathStub('DBC 文件')"><FolderOpen :size="13" />选择 DBC</button><button class="primary" @click="reloadDbc"><RefreshCw :size="13" />重载 DBC</button><button @click="showToast('5 条 DBC 语义覆盖规则已在当前卡片展开显示')"><FileText :size="13" />覆盖规则</button><button @click="viewMessages"><ListTree :size="13" />报文列表</button></div>
        </div>
      </article>
      ...
      <article class="settings-card storage-card">
        ...
        <div class="storage-content card-scroll">
          <label><span>报告目录</span><div class="path-input"><input v-model="settings.storage.report_directory" @input="syncStorageReportPath" /><button aria-label="选择报告目录" @click="pathStub('报告目录')"><FolderOpen :size="13" /></button></div></label>
          <label><span>原始 CAN 日志</span><div class="path-input"><input v-model="settings.storage.raw_can_directory" @input="markDirty" /><button aria-label="选择原始 CAN 日志目录" @click="pathStub('原始 CAN 日志目录')"><FolderOpen :size="13" /></button></div></label>
          <label><span>解码信号日志</span><div class="path-input"><input v-model="settings.storage.decoded_signal_directory" @input="markDirty" /><button aria-label="选择解码信号目录" @click="pathStub('解码信号日志目录')"><FolderOpen :size="13" /></button></div></label>
    ...
    <section class="settings-row-three">
      <article class="settings-card maintenance-card">
        ...
        <div class="maintenance-content">
          ...
          <div class="feature-list">
            <button v-for="feature in featureRows" :key="feature.key" :class="['feature-row',feature.risk&&'risk',feature.permanent&&'permanent']" @click="toggleFeature(feature)"><span><component :is="feature.icon" :size="14" />{{ feature.label }}</span><small>{{ feature.note }}</small><i :class="{ on:Boolean(settings.maintenance[feature.key]) }"><b></b></i></button>
          </div>
          <div class="maintenance-actions"><button class="mock" @click="openDialog('mock')"><Bot :size="14" />{{ settings.maintenance.mock_can_gateway?'关闭 Mock':'启用 Mock' }}</button><button :class="settings.maintenance.maintenance_mode?'exit':'enter'" @click="toggleMaintenance"><ShieldAlert :size="14" />{{ settings.maintenance.maintenance_mode?'退出维护模式':'进入维护模式' }}</button></div>
        </div>
      </article>
      ...
      <article class="settings-card safety-card">
        ...
        <div class="safety-actions"><button class="danger-outline" @click="openDialog('restore')"><RotateCcw :size="13" />恢复安全默认</button><button @click="showToast('版本详情已在当前卡片完整展示')"><Info :size="13" />查看版本详情</button></div>
      </article>
    </section>

    <footer class="settings-action-bar">
      <ActionButton tone="primary" title="保存设置" subtitle="保存所有配置变更" :icon="Save" @click="saveSettings" />
      <ActionButton title="导入配置" subtitle="导入 YAML / JSON 配置" :icon="Upload" @click="importConfig" />
      <ActionButton title="导出配置" subtitle="导出当前配置快照" :icon="Download" @click="exportConfig" />
      <ActionButton title="重载 DBC" subtitle="重新扫描并加载 assets" :icon="RefreshCw" @click="reloadDbc" />
      <ActionButton tone="warning" :title="settings.maintenance.mock_can_gateway?'关闭 Mock':'启用 Mock'" subtitle="切换 MockCanGateway" :icon="Bot" @click="openDialog('mock')" />
      <ActionButton tone="warning" :title="settings.maintenance.maintenance_mode?'退出维护模式':'进入维护模式'" subtitle="需管理员二次确认" :icon="ShieldAlert" @click="toggleMaintenance" />
      <ActionButton tone="danger" title="恢复安全默认" subtitle="仅恢复安全相关参数" :icon="RotateCcw" @click="openDialog('restore')" />
    </footer>
...
import type { SystemMaintenanceSettings, SystemThresholdSetting } from '../api/types'
...
const ActionButton = defineComponent({
  props:{tone:{type:String,default:''},title:{type:String,required:true},subtitle:{type:String,required:true},icon:{type:[Object,Function] as PropType<Component>,required:true}},emits:['click'],
  setup(props,{emit}){return()=>h('button',{class:['action-button',props.tone],onClick:()=>emit('click')},[h('span',{class:'action-icon'},[h(props.icon,{size:23})]),h('span',{class:'action-copy'},[h('strong',props.title),h('small',props.subtitle)])])},
})
...
function pathStub(name:string){showToast(`${name}选择器已预留，当前可直接编辑路径`)}
...
async function importConfig(){await runOperation(()=>settingsStore.importConfig())}
...
@media(max-height:800px){.system-settings-page{grid-template-rows:52px 232px 230px 270px 78px;overflow:auto;scrollbar-color:#2B5C90 #07172A}}
</style>
