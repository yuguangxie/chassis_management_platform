  ...
  <section class="history-page">
    ...
    <article class="panel filter-panel">
      ...
      <div class="filter-line second-line">
        ...
        <div class="filter-actions"><button class="primary" :disabled="historyStore.loading" @click="queryHistory"><Search :size="15" />查询</button><button :disabled="historyStore.loading" @click="resetFilters"><RotateCcw :size="15" />重置</button><button v-if="auth.can('operator')" :disabled="historyStore.loading || historyStore.offline" @click="exportHistory"><Download :size="15" />导出历史</button></div>
      </div>
    </article>
...
const trendOption = computed(() => {
  ...
  return { backgroundColor:'transparent',color:['#21C55D','#EF4444'],tooltip:{trigger:'axis'},legend:{top:0,right:8,textStyle:{color:'#CFE2FF',fontSize:10},itemWidth:14},grid:{left:34,right:14,top:28,bottom:24,containLabel:true},xAxis:{type:'category',data:xAxis,boundaryGap:false,axisLabel:{color:'#8CA6C5',fontSize:9},axisLine:{lineStyle:{color:'#315A83'}},splitLine:{show:true,lineStyle:{color:'#143050',type:'dashed'}}},yAxis:{type:'value',min:0,max:20,axisLabel:{color:'#8CA6C5',fontSize:9},splitLine:{lineStyle:{color:'#1A385C',type:'dashed'}}},series:[{name:'PASS',type:'line',smooth:false,data:pass,symbol:'circle',symbolSize:6,label:{show:true,position:'top',color:'#DDEBFF',fontSize:9},lineStyle:{width:2}},{name:'FAIL',type:'scatter',data:fail.map((value,index)=>value? [xAxis[index],value]:null).filter(Boolean),symbolSize:7,label:{show:true,position:'top',color:'#EF4444',fontSize:9}}] }
})
