  ...
  <section class="history-page">
    <header class="page-title-row">
      ...
      <div class="title-actions">
        <PageDataState :loading="historyStore.loading" :error="historyStore.error" :empty="!historyStore.loading && !dashboard.sessions.length" :stale="historyStore.offline || dashboard.quality !== 'good'" />
        <span v-if="historyStore.offline || dashboard.mock" class="mock-badge">{{ historyStore.offline ? '后端离线，当前使用模拟历史数据' : '显式模拟数据（Mock）' }}</span>
        ...
        <select v-model="refreshSeconds" aria-label="刷新间隔" @change="onIntervalChange"><option value="10">10 秒</option><option value="30">30 秒</option><option value="60">60 秒</option><option value="0">关闭</option></select>
      </div>
    </header>
    ...
    <article class="panel filter-panel">
      <div class="filter-line first-line">
        ...
        <label><span>车辆识别码（VIN）</span><input v-model="filters.vin" placeholder="请输入车辆识别码" /></label>
        ...
        <label><span>结果</span><select v-model="filters.result"><option value="全部">全部</option><option value="PASS">通过</option><option value="FAIL">失败</option><option value="ABORTED">已中止</option><option value="RUNNING">运行中</option></select></label>
      </div>
      <div class="filter-line second-line">
        <label><span>操作员</span><select v-model="filters.operator"><option>全部</option><option v-for="operator in operatorOptions" :key="operator">{{ operator }}</option></select></label>
        <label><span>工位</span><select v-model="filters.station"><option>全部</option><option v-for="station in stationOptions" :key="station">{{ station }}</option></select></label>
        <div class="filter-actions"><IndustrialButton compact variant="primary" :disabled="historyStore.loading" @click="queryHistory"><template #icon><Search :size="15" /></template>查询</IndustrialButton><IndustrialButton compact :disabled="historyStore.loading" @click="resetFilters"><template #icon><RotateCcw :size="15" /></template>重置</IndustrialButton><IndustrialButton v-if="auth.can('operator')" compact :disabled="historyStore.loading || historyStore.offline" @click="exportHistory"><template #icon><Download :size="15" /></template>导出历史</IndustrialButton></div>
      </div>
    </article>
    ...
    <section class="history-main-row">
      <article class="panel session-panel">
        ...
        <div class="table-scroll"><table class="dense-table session-table"><thead><tr><th>会话编号</th><th>底盘编号</th><th>车辆识别码（VIN）</th><th>开始时间</th><th>结束时间</th><th>结果</th><th>失败步骤</th><th>操作员</th><th>报告</th></tr></thead><tbody>
    ...
    <section class="history-chart-row">
      ...
      <article class="panel session-actions-panel"><h2>会话操作</h2><div class="session-actions"><IndustrialActionButton title="打开会话详情" subtitle="查看完整步骤与断言" @click="openDetail"><template #icon><FolderSearch :size="26" /></template></IndustrialActionButton><IndustrialActionButton variant="success" title="回放曲线" subtitle="多通道信号曲线回放" @click="replayCurve"><template #icon><ChartNoAxesCombined :size="26" /></template></IndustrialActionButton><IndustrialActionButton variant="neutral" title="下载数据包" subtitle="下载所选会话全部数据" @click="downloadBundle"><template #icon><Download :size="26" /></template></IndustrialActionButton></div></article>
    </section>

    <footer class="history-footer">
      <div class="summary-stats"><div><span>累计检测</span><strong class="green">{{ dashboard.summary.total_tests.toLocaleString() }}</strong></div><div><span>通过率</span><strong class="green">{{ dashboard.summary.pass_rate }}%</strong></div><div><span>失败数</span><strong class="red">{{ dashboard.summary.fail_count }}</strong></div><div><span>告警总数</span><strong class="red">{{ dashboard.summary.alarm_count }}</strong></div><div><span>当前筛选结果</span><strong class="green">{{ dashboard.summary.filtered_count }} <small>条</small></strong></div></div>
...
const selected = computed(() => historyStore.offline ? emptySelected : (dashboard.value.selected_session || emptySelected))
...
const filters = reactive({ chassisNo: '', vin: '', serialNo: '', startTime: '', endTime: '', result: '全部', operator: '全部', station: '全部', page: 1, pageSize: 20 })
...
const operatorOptions = computed(() => [...new Set(dashboard.value.sessions.map((item) => item.operator).filter(Boolean))].sort())
const stationOptions = computed(() => [...new Set(dashboard.value.sessions.map((item) => item.station_id).filter((item): item is string => Boolean(item)))].sort())

const trendOption = computed(() => {
  const chart = dashboard.value.charts.vehicle_result_trend
  const xAxis = historyStore.offline ? [] : chart.x_axis
  const pass = historyStore.offline ? [] : chart.pass
  const fail = historyStore.offline ? [] : chart.fail
  return { backgroundColor:'transparent',color:['#21C55D','#EF4444'],tooltip:{trigger:'axis'},legend:{type:'scroll',top:0,right:8,textStyle:{color:'#CFE2FF',fontSize:10},itemWidth:14},grid:{left:34,right:14,top:28,bottom:24,containLabel:true},xAxis:{type:'category',data:xAxis,boundaryGap:false,axisLabel:{color:'#8CA6C5',fontSize:9,hideOverlap:true},axisLine:{lineStyle:{color:'#315A83'}},splitLine:{show:true,lineStyle:{color:'#143050',type:'dashed'}}},yAxis:{type:'value',min:0,max:20,axisLabel:{color:'#8CA6C5',fontSize:9,hideOverlap:true},splitLine:{lineStyle:{color:'#1A385C',type:'dashed'}}},series:[{name:'通过',type:'line',smooth:false,data:pass,symbol:'circle',symbolSize:6,label:{show:true,position:'top',color:'#DDEBFF',fontSize:9},lineStyle:{width:2}},{name:'失败',type:'scatter',data:fail.map((value,index)=>value? [xAxis[index],value]:null).filter(Boolean),symbolSize:7,label:{show:true,position:'top',color:'#EF4444',fontSize:9}}] }
})

const paretoOption = computed(() => {
  const chart = dashboard.value.charts.failure_pareto
  const categories = historyStore.offline ? [] : chart.categories
  const counts = historyStore.offline ? [] : chart.counts
  const cumulative = historyStore.offline ? [] : chart.cumulative_percent
  return { backgroundColor:'transparent',color:['#2F80FF','#F6C343'],tooltip:{trigger:'axis'},legend:{top:0,right:8,textStyle:{color:'#CFE2FF',fontSize:10},itemWidth:14},grid:{left:34,right:36,top:28,bottom:30,containLabel:true},xAxis:{type:'category',data:categories,axisLabel:{color:'#8CA6C5',fontSize:9,interval:0},axisLine:{lineStyle:{color:'#315A83'}}},yAxis:[{type:'value',min:0,max:15,axisLabel:{color:'#8CA6C5',fontSize:9},splitLine:{lineStyle:{color:'#1A385C',type:'dashed'}}},{type:'value',min:0,max:100,axisLabel:{formatter:'{value}%',color:'#F6C343',fontSize:9},splitLine:{show:false}}],series:[{name:'失败次数',type:'bar',barWidth:'38%',data:counts,label:{show:true,position:'top',color:'#DDEBFF',fontSize:9}},{name:'累计占比',type:'line',yAxisIndex:1,data:cumulative,symbol:'circle',symbolSize:5,label:{show:true,formatter:'{c}%',position:'top',color:'#F6C343',fontSize:9},lineStyle:{width:2}}] }
})
...
function currentQuery(): HistoryQuery { return { chassis_no:filters.chassisNo,vin:filters.vin,serial_no:filters.serialNo,start_time:filters.startTime,end_time:filters.endTime,result:filters.result,operator:filters.operator,station:filters.station,page:filters.page,page_size:filters.pageSize } }
async function queryHistory(){ filters.page=1; await historyStore.loadDashboard(currentQuery()); showToast(historyStore.error ? `查询失败：${historyStore.error}` : `查询完成：共 ${dashboard.value.pagination.total} 条`) }
async function resetFilters(){ Object.assign(filters,{chassisNo:'',vin:'',serialNo:'',startTime:'',endTime:'',result:'全部',operator:'全部',station:'全部',page:1,pageSize:20}); await historyStore.loadDashboard(currentQuery()); showToast(historyStore.error ? `重置失败：${historyStore.error}` : '筛选条件已重置') }
...
<style scoped>
.history-page{position:relative;height:100%;min-height:0;overflow:hidden;display:grid;grid-template-rows:42px 100px 300px 180px minmax(0,1fr) 62px;gap:10px;color:#EAF2FF}.panel{min-height:0;overflow:hidden;border:1px solid #1E3A5F;border-radius:8px;background:linear-gradient(180deg,rgba(16,36,61,.98),rgba(8,23,41,.98));box-shadow:0 9px 22px rgba(0,0,0,.2)}
...
.chart-panel{display:grid;grid-template-rows:27px minmax(0,1fr);padding:0 8px 7px}.chart-panel h2{display:flex;align-items:center}.session-actions-panel{display:grid;grid-template-rows:30px minmax(0,1fr);padding:0 10px 10px}.session-actions-panel h2{display:flex;align-items:center}.session-actions{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));align-content:center;gap:10px}.session-actions :deep(.industrial-action){width:100%;padding-inline:8px;gap:7px}.session-actions :deep(.action-copy strong){font-size:11px}.session-actions :deep(.action-copy small){font-size:8px}
...
.filter-actions button:disabled,.download-cards button:disabled,.session-actions button:disabled{opacity:.45;cursor:not-allowed}
...
@media(max-height:800px){.history-page{grid-template-rows:42px 82px minmax(190px,1.4fr) minmax(118px,.85fr) minmax(116px,.85fr) 52px;gap:7px;overflow:hidden}.filter-panel{padding-block:5px}.filter-line{gap:8px}.filter-line input,.filter-line select,.time-range div{height:26px}.filter-actions button{height:28px;min-width:88px}.summary-stats>div{height:36px}.timeline-item{min-height:25px}.download-cards button{padding:6px}}
</style>
